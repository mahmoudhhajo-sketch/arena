...
// Keep GoblinBoyZ an orc club while retaining its five goblin recruits.
export async function initializeRevision20(){
 if(await record('revision-v20-goblinboyz-orc-club'))return;
 await db.transaction(async(tx:any)=>{
  ...
  if(team){
   ...
   await tx.update(clubs).set({race:'orc',hometown:'Skrothåla'}).where(eq(clubs.id,team.id));
  }
  await put('system','revision-v20-goblinboyz-orc-club',{date:new Date().toISOString()},tx);
 });
}
