 ...
 for(const event of due){
  ...
  await db.transaction(async(tx:any)=>{await tx.execute(sql`SELECT pg_advisory_xact_lock(719084)`);const a=await record(auction.id,tx);if(a.closed||+new Date(a.endsAt)>+now)return;
   if(a.bidderId){if(a.unreserved)await tx.update(clubs).set({gold:sql`${clubs.gold}-${a.price}`}).where(eq(clubs.id,a.bidderId));const squad=await tx.select().from(players).where(eq(players.clubId,a.bidderId));const nums=new Set(squad.map((p:any)=>p.shirtNumber));let number=1;while(nums.has(number))number++;
    ...
    const bought=(await tx.select().from(players).where(eq(players.id,a.playerId)))[0],buyer=(await tx.select().from(clubs).where(eq(clubs.id,a.bidderId)))[0];await put('morale-change','morale-'+a.id,{clubId:a.bidderId,date:event.date,penalty:bought.race===buyer.race?1:3},tx);await put('ledger','purchase-'+a.id,{clubId:a.bidderId,category:'Spelarköp',amount:-a.price,date:event.date},tx);
   }
  ...
  await db.transaction(async(tx:any)=>{const s=await record(scout.id,tx);if(!s.destination)return;const [club]=await tx.select().from(clubs).where(eq(clubs.id,s.clubId));const place=PLACES.find(p=>p.id===s.destination)!;
   ...
   const [created]=await tx.insert(players).values({...playerRow(p,club.id),createdAt:new Date(event.date)}).returning();await teamNews(tx,club.id,'scout-'+created.id,'Talangscouten har hittat en spelare',created.name+' har anslutit från '+place.name+'.',{kind:'player',id:created.id});
   await put('potential','potential-'+place.id,{value:Math.max(0,potential-2),date:event.date},tx);
   await put('scout',s.id,{...s,destination:null,locationId:place.id,returnsAt:null,message:'Talangscouten hittade '+created.name+' i '+place.name+'.',lastPlayerId:created.id},tx);
  });
 }
 }
...
export function registerExpansion(app:Express){
 ...
 route(app,'post','/api/market/list',async(req)=>db.transaction(async(tx:any)=>{
  ...
  await put('auction','auction-'+randomUUID(),{playerId:p.id,sellerId:p.clubId,price,bidderId:null,endsAt:new Date(Date.now()+7*86400000).toISOString(),closed:false},tx);return{success:true};
 }));
 ...
 route(app,'post','/api/forum',async(req)=>{const {clubId,category,text,title,threadId}=req.body;const [club]=await db.select().from(clubs).where(eq(clubs.id,clubId));const parent=threadId?await record(threadId):null;if(!club||!String(text||'').trim()||String(text).length>5000||![...DIVISION_NAMES,'Allmänt','Spelfrågor','Transfer','Förslag'].includes(category)||threadId&&(!parent||parent.threadId||parent.category!==category)||!threadId&&!String(title||'').trim())throw Error('Kontrollera rubrik, kategori och meddelande.');const id='post-'+randomUUID();await put('forum',id,{category,threadId:threadId||null,text:String(text).trim(),title:parent?.title||String(title).slice(0,120),author:club.ownerName,clubId:club.id,date:new Date().toISOString()});return{id:threadId||id};});
