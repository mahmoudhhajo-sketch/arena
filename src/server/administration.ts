...
export function registerAdministration(app:Express){
 ...
 app.get('/api/administration',async(_req,res)=>{try{
  const teams=await db.select().from(clubs),accounts=await records('account'),bans=await records('moderation-account'),roles=await records('admin-role'),loginEvents=await records('login-event'),sessions=await records('session'),now=Date.now();
  ...
  const loginStats=accounts.map(a=>{const events=loginEvents.filter(e=>e.userId===a.userId).sort((x,y)=>String(y.date).localeCompare(String(x.date))),online=sessions.some(s=>s.userId===a.userId&&!s.guest&&s.expires>now&&Number(s.lastSeen||0)>=now-5*60000);return {userId:a.userId,name:a.name,managerName:a.managerName||a.name,clubName:teams.find(c=>c.userId===a.userId)?.name||'Inget lag',lastLogin:events[0]?.date||null,total:events.length,online};}).sort((a,b)=>Number(b.online)-Number(a.online)||String(b.lastLogin||'').localeCompare(String(a.lastLogin||'')));
  const today=new Date(now);today.setHours(0,0,0,0);const week=now-7*86400000;
  res.json({clubs:teams.map(c=>({id:c.id,name:c.name,shortName:c.shortName,race:c.race,hometown:c.hometown,ownerName:c.ownerName,isBot:c.isBot,gold:c.gold,division:c.division})),accounts:accounts.map(a=>({userId:a.userId,name:a.name,managerName:a.managerName||a.name,clubName:teams.find(c=>c.userId===a.userId)?.name,admin:roles.some(r=>r.userId===a.userId&&r.enabled),banned:bans.find(b=>b.userId===a.userId)?.banned||false})),loginStats,loginSummary:{online:loginStats.filter(x=>x.online).length,today:loginEvents.filter(e=>+new Date(e.date)>=+today).length,week:loginEvents.filter(e=>+new Date(e.date)>=week).length,total:loginEvents.length},forum:(await records('forum')).sort((a,b)=>b.date.localeCompare(a.date)).slice(0,500),chat:await db.select().from(shoutboxMessages).orderBy(desc(shoutboxMessages.createdAt)).limit(100),auctions:auctions.filter(a=>!a.closed).map(a=>({...a,sellerName:teams.find(c=>c.id===a.sellerId)?.name||'Kejsaren',bidderName:teams.find(c=>c.id===a.bidderId)?.name||'Inget bud'})),content:await records('content-page'),contentVersions:(await records('content-version')).sort((a,b)=>b.date.localeCompare(a.date)).slice(0,100),audit:(await records('admin-audit')).sort((a,b)=>b.date.localeCompare(a.date)).slice(0,100)});
