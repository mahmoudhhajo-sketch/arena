...
// Daily stock is limited separately from income: expensive spells remain a choice.
export function manaPlan(budget:number,level:number,reference:number){
 const capacity=[0,60,110,180][Math.max(0,Math.min(3,level))];
...
export function regenerateMana(state:any,budget:number,level:number,reference:number,now=Date.now()){
 ...
 const mana=Math.min(plan.capacity,Math.max(0,Number(state?.mana||0))+days*Number(previous?.dailyGain||0));
